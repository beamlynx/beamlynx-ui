...
- MCP: a reveal request (an agent asking to see something its access policy redacted) now also gets its own pinned tab at the end of the strip, marked with an eye icon, instead of opening as a new tab mixed in with your own and stealing focus to get there. Several pending requests each get their own pinned tab. The tab disappears once you reveal or decline; closing it without deciding declines it, so an agent waiting on a decision never hangs indefinitely.
