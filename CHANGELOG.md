...
## [Unreleased]
### Added
- A tab can now explain itself. Start an expression with a comment -- either `/* ... */` across lines, or a run of `--` lines -- and the canvas shows it in its top-left corner instead of leaving it as grey text to scroll past. Write what you are looking for and why when you open a tab, and it is still there when you come back to it. Canvas gestures keep the comment rather than deleting it on the next click. Needs a pine-lang server that returns the comment; against an older one, nothing changes.
- You can write that comment from the canvas itself, like any other canvas edit: press `c` (shown at the front of the bottom-left keybinding legend), click the comment button in the canvas toolbar, or click an existing comment to edit it. A dim "Add a comment" marks the spot when there isn't one yet. What you type is spliced into the Pine expression as a comment, exactly the way picking a table or a column splices in an operation -- so the canvas and the expression can never disagree. Emptying it removes the comment. Enter is a line break; Cmd/Ctrl+Enter or clicking away saves, Esc cancels.
...
- MCP: when an agent asks to see something its access policy redacted, that request now gets its own "Needs approval" pinned tab at the end of the strip, instead of opening as a new tab mixed in with your own and stealing focus to get there. Several pending requests each get their own pinned tab. The tab shows which connection the request is against, the agent's own reason for asking, and Approve/Decline. It disappears once you decide; closing it without deciding declines the request, so an agent waiting on a decision never hangs indefinitely.
- MCP: an agent icon in the header, next to the bell, covers everything the agent is doing. It shows a dot when a query result landed while you were looking elsewhere, and a count when approvals are waiting on you. Clicking it goes to whichever needs you most: an approval first, since an agent is blocked waiting on your decision, otherwise the activity tab.
...
- Results columns size to their own content instead of stretching to fill the pane. A column now keeps a fixed width based on a sample of its actual values, so a UUID column gets the room it needs and a short "status" column doesn't get stretched to match -- scroll horizontally if a wide result doesn't fit, the same as any spreadsheet. This is also what made the remaining panel-animation stutter with a populated grid: MUI's `flex` columns recalculate every column's width across every visible row on every resize; fixed columns cost nothing to resize.
- Table colors, and hovering a table on the canvas, now tint each column's header only, not every value under it. Coloring every cell read as noise across a full table of rows -- and the hover spotlight was doing this even with "Table colors" turned off, which made the preference look like it had no effect.
...
- Upgraded the results grid library (`@mui/x-data-grid` 7.15.0 -> 7.29.13, 14 minor releases of fixes) after measuring a real, repeatable ~18% cut in the main-thread work it does when its container resizes -- on top of, not instead of, the placeholder masking below.
- The results-settling placeholder now shows a proper skeleton -- your real column headers and their table colors, with shimmering row-shaped bars underneath -- instead of a blank body.
- The results grid no longer just looks smoother during a panel's resize - the whole app stays responsive while it happens. It's fully unmounted (not merely covered) for that one moment and remounts fresh once the resize settles, which measured as both cheaper overall and, unlike covering it, doesn't leave every other click and hover on the page waiting behind it. A plain header-shaped placeholder -- your columns and their table colors, no data rows -- stands in for the instant it's gone.
- Opening or closing Settings no longer re-centers the canvas. It never needed to: Settings takes its space from the results pane, not the canvas, whose width holds constant throughout. The earlier fix that added this was based on a wrong assumption and was quietly redoing a real (if small) chunk of work -- a full graph re-fit -- on every single toggle for no visible reason.
- Resizing a results column by hand no longer snaps back to its default width. The results grid was rebuilding its whole column list as a new object on every render -- including ones that had nothing to do with the grid, like hovering a table on the canvas -- and losing whatever width you'd just dragged in the process.
