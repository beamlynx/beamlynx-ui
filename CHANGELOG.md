...
- MCP: when an agent asks to see something its access policy redacted, that request now gets its own "Needs approval" pinned tab at the end of the strip, instead of opening as a new tab mixed in with your own and stealing focus to get there. Several pending requests each get their own pinned tab. The tab disappears once you reveal or decline; closing it without deciding declines it, so an agent waiting on a decision never hangs indefinitely.
- MCP: the agent icon in the header now covers pending approvals too, not just query activity, and shows how many are waiting. Clicking it goes to whichever needs you most -- an approval first, since an agent is blocked waiting on your decision, otherwise the activity tab. It also appears for an approval that arrives before the agent has run any query, which previously left the header showing nothing at all.
