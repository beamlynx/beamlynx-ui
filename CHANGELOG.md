...
- Table colors now tint each column's header only, not every value under it. Coloring every cell read as noise across a full table of rows; the header alone already says which table a column came from. Hovering a table on the canvas still spotlights its cells too, same as before -- that's a separate, temporary highlight, not the always-on tint.
