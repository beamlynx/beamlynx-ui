...
- A parent node's own referenced column now shows a "•" placeholder on its handle even before any child relation is confirmed, instead of leaving it blank. pine-lang has no primary-key concept at all — even a confirmed join only tells us the *child's* FK column, never the parent's own — so this is a neutral stand-in for "there's a real column here" rather than a specific guess.
- A suggested node's column caption wasn't positioned to align with its own handle (it rendered as a plain line under the table name, unrelated to wherever the handle happened to sit) — and briefly overlapped the table name after the caption was made position-aware, from an incorrect assumed header height. Both the caption and its handle now anchor to the same measured row.
- A suggested node showed a connection dot on both sides even though it only ever has one real relation to context — a child suggestion now only shows its left (target) handle, a parent suggestion only its right (source) handle.
- A node's anonymous, unlabeled relation handle (the side with no confirmed relation yet, only unconfirmed suggestions) sat at the node's plain vertical center instead of row 0's position, so it didn't line up with the other side once that side had labeled rows and the node grew taller to fit them.
