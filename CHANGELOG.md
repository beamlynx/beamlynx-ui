...
### Fixed
- A parent node's own referenced column (virtually always "id") now shows on its handle even before any child relation is confirmed, instead of leaving it blank. pine-lang has no primary-key concept at all — even a confirmed join only tells us the *child's* FK column — so this is a hardcoded "id" guess standing in until real derivation is figured out, not derived data.
