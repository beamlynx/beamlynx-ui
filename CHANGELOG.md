...
- The results grid no longer visibly stutters mid-transition when a panel opens or closes beside it. It still has to redo some work once the resize settles -- MUI's own cost, not something we can remove outright -- but that work now happens behind a plain placeholder instead of in view, so nothing choppy is left to watch.
