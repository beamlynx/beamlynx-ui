...
## [Unreleased]
### Added
- A new Settings -> Access Policy section: create, rename, and delete named access policies, each its own toggleable set of rule modules deciding which columns come back as their real value (instead of a fixed `xxxxx` placeholder) -- real Postgres types (UUIDs, numbers, dates, times, booleans, enums), identifier/reference columns (foreign keys, and columns a naming convention strongly suggests are one), and columns named `*_id`. Built to grow: adding a rule later is a new row in this section, not a redesign.
- Each saved connection (Settings -> Database Connections) now has a policy picker alongside "Enable for MCP access" -- select which policy applies to it, or "None". **MCP always has a real, active policy behind it once enabled: there is no "reachable but unprotected" state.** "Enable for MCP access" is disabled, with an explanatory tooltip, until the connection's own selected policy has an active rule; "None" is disabled in the policy picker for as long as MCP stays on for that connection, rather than letting a click land on a state that would just be refused.
- A second, independent toggle next to the policy picker -- "Only apply to MCP server" -- lets a connection's owner see real data on their own (non-MCP) queries against it, e.g. while debugging, without ever weakening what the MCP agent sees on the same connection. Off by default (protected by default, same as the rest of this feature).
- A redacted column returns a fixed `xxxxx` placeholder from pine-lang directly in the generated SQL, so the real value never leaves the database. Applies to every query against a connection with an active policy selected, from any tab, not just the dedicated MCP tab. Known tradeoff: grouping by a redacted column (e.g. `group: email => count`) still runs, but every group collapses under the one placeholder value -- expected, not a bug.

### Fixed
- An MCP query now always re-reads every access policy (and the connection's own selections) fresh before running, instead of trusting whatever `GlobalStore` happened to hold since the app last loaded it (app boot, or a connect/disconnect). An edit made while the app was already running used to have no effect until something unrelated happened to refresh that state, with no error to explain why. `runMcpQuery`/`explainMcpQuery` (`store/mcp-query.ts`) both resolve the policy through `resolveAccessPolicyRules`, which refreshes first.
- `McpBridge.tsx`'s effect now unsubscribes its previous MCP-query listener before registering a new one (desktop's `mcp.onQueryRequest` now returns an unsubscribe function to support this). It didn't before, so every re-run of the effect (a Fast Refresh during dev, or React Strict Mode's double-invoke) left the old listener running alongside the new one -- every listener still answers every request, so which one's response actually got used was a race, occasionally surfacing as an unexplained "X is not a function" from a stale listener closing over code from before some method existed.
- Canvas keybindings (`s`/`w`/`o`/`g`/`x`/`u`/`U`/`i`) stopped working with Settings open, after picking a table/join or submitting a where-value from the canvas picker -- and stayed stuck until clicking the canvas again. The picker's own filter/value input is auto-focused while open; closing it after a single pick removes that input from the DOM while it still holds focus, and the resulting native blur has nowhere to send focus. With no compensating focus target, keyboard-panel tracking fell back to Settings whenever it happened to be open. `Picker.tsx` now explicitly returns focus to the canvas the instant it closes, rather than leaving it to the browser.
...
### Fixed
- The canvas no longer re-adds a `|= agg` name to a `group:`/`limit:` step after you deliberately remove it (e.g. by editing the raw Pine text) -- it used to reappear the moment any other edit landed, because a background check couldn't tell "just grouped, never named yet" from "name removed on purpose". The checkpoint is now named only on demand, the first time you use one of its select/where/order/join actions, with a "loading..." indicator on the frame while that resolves.
- Fixed the resulting first-click issue: that first select/where/order/join click on a freshly-named checkpoint used to fetch its dropdown values against the *previous*, unnamed shape of the query, showing wrong or empty results until a second click caught up. The canvas now has the correct shape in hand immediately once naming finishes, so the first click already shows the right values.
...
- `j`/`k`/Arrow Up/Down stopped moving Settings' rail the instant Tab landed on a toggle switch (e.g. in Preferences) -- the guard meant to leave real typing alone matched any `<input>`, including a Switch's own underlying checkbox, which a letter or arrow key does nothing to anyway. Now only genuine text/number/date-style inputs hold onto the keystroke.
